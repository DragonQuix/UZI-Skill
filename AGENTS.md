...
## 🗺️ Repository Layout & Entrypoints (v3.13.1)
...
| v3.13.1 | 插件本地源切换经验沉淀 · Codex/Claude Code marketplace 指向本 fork 工作树 | 后续推送后按 §插件本地源切换与更新 同步；不改默认发布流程 |
...
4. WRITTEN agent_analysis.json with dim_commentary + qualitative_deep_dive + panel_insights + overrides
5. SET agent_reviewed: true in agent_analysis.json
6. CONFIRMED qualitative_deep_dive covers all 6 dims (3_macro/7_industry/8_materials/9_futures/13_policy/15_events)
</HARD-GATE>
...
**批量处理**：同组内 signal 相同且理由一致的投资者可共用 reasoning，无需每人独立编写。例如游资组 22/23 人因市值过大 skip——一条 "市值 >1000 亿不适合游资打法" 即可覆盖，不必重复。
...
```json
{
  ...
  "qualitative_deep_dive": {
    "3_macro": { "evidence": [{"source":"...", "url":"...", "finding":"...", "retrieved_at":"..."}], "conclusion": "..." },
    "7_industry": { "evidence": [...], "conclusion": "..." },
    "8_materials": { "evidence": [...], "conclusion": "..." },
    "9_futures": { "evidence": [...], "conclusion": "..." },
    "13_policy": { "evidence": [...], "conclusion": "..." },
    "15_events": { "evidence": [...], "conclusion": "..." }
  },
  "panel_insights": "整体评委观察（30字以上，概括投票结构和主要分歧）",
  "notable_overrides": [
    {"investor": "索普", "from": "skip/0", "to": "neutral/50", "why": "规则引擎误判非A股，实际是A股，索普可用凯利公式分析"}
  ],
...
## 🔌 插件本地源切换与更新（v3.13.1 经验沉淀 · 重要）

> 当用户要求"两个平台都用本地最新版本、不要用远程插件更新"时按此节操作。
> 详细备份/回滚步骤见 `CODEX_HOME` 下 `UZI-PLUGIN-LOCAL-SOURCE.md`。

### 触发信号

- 用户说"用本 fork 的最新版本"、"不要用远程插件"、"插件没跟随我的改动"
- Codex / Claude Code 仍在加载停在旧 commit 的缓存（与 `git log` 最新 HEAD 不一致）
- `plugins\cache\uzi-skill\stock-deep-analyzer\3.13.0` 不含本仓库最新改动

### 根因

两个平台默认从远程 fork（如 `wbh604/UZI-Skill`）克隆插件，缓存在 `.cache` 与
`plugins/cache` 下，不跟随本机工作树 `E:\Repos\UZI-Skill` 推进；推送后缓存不动。

### Codex 侧正确做法

1. **改 `CODEX_HOME\config.toml` 的 marketplace source**
   `[marketplaces.uzi-skill].source` 从 `vendor_imports\skills\UZI-Skill`（远程
   fork 副本）改为 `\\?\E:\Repos\UZI-Skill`（本 fork 工作树）。参考 `[marketplaces.ecc]`
   已有本地路径模式。
2. **禁用重复插件**：`[plugins."stock-deep-analyzer@local-personal"]` 设
   `enabled = false`，避免 `@uzi-skill` 与 `@local-personal` 双源加载同插件。
3. **清理旧 cache 触发重装**：删除 `plugins\cache\uzi-skill\stock-deep-analyzer\<ver>`
   与 `plugins\cache\local-personal\stock-deep-analyzer\<ver>`，Codex 重启后按新
   source 重建。
4. **改前先备份** `config.toml` 到 `*.bak-uzi-local-<date>`。

### Claude Code 侧正确做法

1. **marketplace 源已是指向本 fork 的 git clone**：`local-plugins\uzi-skill`
   remote 为 `DragonQuix/UZI-Skill`。只需 `git pull --ff-only` 到最新 HEAD。
2. **同步实际加载的 cache**（cache 不会自动跟踪源）：
   ```powershell
   $src = "C:\Users\admin\.claude\local-plugins\uzi-skill"
   $dst = "C:\Users\admin\.claude\plugins\cache\uzi-skill\stock-deep-analyzer\<ver>"
   robocopy $src $dst /E /XD ".git" "__pycache__" "reports" ".cache" "prewarm" /XF "*.pyc" ".env" /NFL /NDL /NJH /NJS /NP
   ```
   退出码 1 表示成功复制（非错误）；不传 `/MIR` 避免误删 cache 独有的运行时产物。
3. **更新 `installed_plugins.json`**：把 `stock-deep-analyzer@uzi-skill` 的
   `gitCommitSha` 更新为 `git rev-parse HEAD`，`lastUpdated` 更新为当前 UTC ISO。
4. **改前先备份** `installed_plugins.json` 到 `*.bak-uzi-local-<date>`。
5. 若 `local-plugins\uzi-skill` 还不是本 fork clone，改 `known_marketplaces.json`
   的 `uzi-skill.source.path` 指向本 fork clone 或重新克隆。

### 后续本 fork 推送新提交后

- **Codex**：无需手动动作，重启后按 source 重建 cache；若插件未刷新，手动删
  `plugins\cache\uzi-skill\` 触发重装。
- **Claude Code**：`local-plugins\uzi-skill` 跑 `git pull --ff-only` 后，再
  robocopy 同步 cache + 更新 `installed_plugins.json` 的 sha；或运行
  `claude plugin update stock-deep-analyzer@uzi-skill` 自动触发重装。

### 验证清单

- Codex `config.toml` 中 `[marketplaces.uzi-skill].source` 指向本 fork
- Codex `@local-personal` 同名插件 `enabled = false`
- 旧 wbh604 cache 两处已删除
- Claude Code `local-plugins\uzi-skill` HEAD 与本 fork 一致
- Claude Code cache 中 `fetch_moat.py` 行数 / 新增文件与源一致
- `installed_plugins.json` 的 `gitCommitSha` 等于本 fork HEAD
- **改完必须提醒用户重启 Codex App / Claude Code 新会话后验证**

### 适用范围 / 边界

- 仅为消除"远程缓存不跟随本 fork"问题；不替代正式 plugin 发布流程。
- `vendor_imports\skills\UZI-Skill`（wbh604 clone）保留不动作历史归档，不再被任何
  marketplace 引用；不要删它以防回滚。
- 不应把本 fork 工作树直接 symlink 到 cache（Windows 权限/锁定风险未知），用 robocopy 副本。
- 改 `CODEX_HOME\config.toml` 后按全局规则提醒新会话验证生效。
